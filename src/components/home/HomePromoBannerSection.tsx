...
export function HomePromoBannerSection({ title, subtitle, imageUrl, targetHref, gradientFrom, gradientTo }: Props) {
  return (
    <section className="bg-[#faf7f8] py-14 sm:py-16">
      <div className="section-shell">
        ...
        >
          <div className="grid min-h-[450px] items-end gap-4 px-6 py-6 sm:grid-cols-[1.05fr_0.95fr] sm:px-10 sm:py-8">
            <div className="max-w-xl text-white">
              <h3 className="font-[family-name:var(--font-heading)] text-3xl font-semibold leading-tight sm:text-4xl">
                {title}
              </h3>
              {subtitle ? <p className="mt-3 text-sm text-white/90 sm:text-base">{subtitle}</p> : null}
              <span className="mt-6 inline-flex items-center rounded-full bg-white/95 px-5 py-2 text-sm font-semibold text-[#7f1530] transition group-hover:bg-white">
                See more
              </span>
            </div>
