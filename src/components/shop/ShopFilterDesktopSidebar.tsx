"use client";

import { useLayoutEffect, useRef, useState, useSyncExternalStore } from "react";
import { X } from "lucide-react";
import type { ShopFilterOptions } from "@/lib/shop-filter-shared";
import { ShopFilters } from "@/components/shop/ShopFilters";
import { useShopFilterSheet } from "@/components/shop/ShopFilterSheetProvider";

function subscribeReducedMotion(notify: () => void) {
  const mq = window.matchMedia("(prefers-reduced-motion: reduce)");
  mq.addEventListener("change", notify);
  return () => mq.removeEventListener("change", notify);
}

function reducedMotionMatches() {
  return window.matchMedia("(prefers-reduced-motion: reduce)").matches;
}

/** Single duration so column width and panel fade move together (open + close). */
const SYNC_TRANSITION_MS = 300;

let oneInPxCache: number | null = null;

/** Matches Tailwind `1in` padding on the scroll shell for footer gutter math. */
function measureOneInPx(): number {
  if (oneInPxCache != null) return oneInPxCache;
  if (typeof document === "undefined") return 96;
  const d = document.createElement("div");
  d.style.cssText =
    "position:absolute;left:-9999px;top:0;height:1in;width:0;visibility:hidden;pointer-events:none";
  document.body.appendChild(d);
  const h = d.getBoundingClientRect().height;
  document.body.removeChild(d);
  oneInPxCache = h > 0 ? h : 96;
  return oneInPxCache;
}

type Dock = { left: number; width: number; top: number; bottom: number };

type Props = {
  options: ShopFilterOptions;
  basePath: string;
  enablePriceSlider?: boolean;
  hideOutOfStockToggle?: boolean;
  preserveQueryKeys?: readonly string[];
  omitUrlFilterKeys?: readonly string[];
  hideCategoryFilter?: boolean;
  hideFacetFields?: readonly ("style" | "occasion" | "material")[];
};

/**
 * Desktop filter column. While open on `lg+`, the scroll shell is `position: fixed` and
 * re-measured on scroll/resize so it tracks the catalog row (`[data-shop-catalog-body]`)
 * and stops above the site footer — `overflow-x-clip` on ancestors breaks pure `sticky`.
 */
export function ShopFilterDesktopSidebar({
  options,
  basePath,
  enablePriceSlider = true,
  hideOutOfStockToggle = true,
  preserveQueryKeys = [],
  omitUrlFilterKeys = [],
  hideCategoryFilter = false,
  hideFacetFields = []
}: Props) {
  const applyFiltersRef = useRef<(() => void) | null>(null);
  const asideRef = useRef<HTMLElement | null>(null);
  const [dock, setDock] = useState<Dock | null>(null);
  const { isFiltersOpen, closeFilters } = useShopFilterSheet();
  const reduceMotion = useSyncExternalStore(subscribeReducedMotion, reducedMotionMatches, () => false);

  useLayoutEffect(() => {
    if (!isFiltersOpen) {
      setDock(null);
      return;
    }

    const el = asideRef.current;
    if (!el) return;

    const lgMq = window.matchMedia("(min-width: 1024px)");

    const minViewportTopPx = () =>
      window.matchMedia("(min-width: 640px)").matches ? 128 : 120; // SiteMainShell sm:pt-[8rem] / pt-[7.5rem]

    const read = () => {
      if (!lgMq.matches) {
        setDock(null);
        return;
      }
      const r = el.getBoundingClientRect();
      if (r.width < 12) {
        setDock(null);
        return;
      }

      const shell =
        el.closest("[data-shop-catalog-body]") ?? el.closest(".section-shell");
      const shellTop =
        shell instanceof HTMLElement ? shell.getBoundingClientRect().top : minViewportTopPx();
      const topPx = Math.max(minViewportTopPx(), shellTop);

      const insetPx = measureOneInPx();
      const minBottomPx = 16;
      const footerEl = document.querySelector<HTMLElement>("[data-site-footer]");
      const footerTop =
        footerEl instanceof HTMLElement ? footerEl.getBoundingClientRect().top : Number.POSITIVE_INFINITY;

      let bottomPx = minBottomPx;
      if (footerEl && footerTop > 0) {
        bottomPx = Math.max(minBottomPx, window.innerHeight - footerTop + insetPx);
      }

      const minScrollPx = 1;
      const maxBottom = window.innerHeight - topPx - minScrollPx;
      if (bottomPx > maxBottom) bottomPx = Math.max(minBottomPx, maxBottom);

      setDock({ left: r.left, width: r.width, top: topPx, bottom: bottomPx });
    };

    read();
    const ro = typeof ResizeObserver !== "undefined" ? new ResizeObserver(read) : null;
    if (ro) ro.observe(el);
    const footerEl = document.querySelector<HTMLElement>("[data-site-footer]");
    if (ro && footerEl) ro.observe(footerEl);
    lgMq.addEventListener("change", read);
    window.addEventListener("resize", read);
    window.addEventListener("scroll", read, { passive: true });
    return () => {
      if (ro) ro.disconnect();
      lgMq.removeEventListener("change", read);
      window.removeEventListener("resize", read);
      window.removeEventListener("scroll", read);
    };
  }, [isFiltersOpen]);

  const easeColumn = "cubic-bezier(0.22, 1, 0.36, 1)";
  const easePanel = "cubic-bezier(0.32, 0.72, 0, 1)";

  const columnStyle = reduceMotion
    ? undefined
    : {
        transitionProperty: "width, max-width, margin-inline-end",
        transitionDuration: `${SYNC_TRANSITION_MS}ms`,
        transitionTimingFunction: easeColumn
      };

  const panelStyle = reduceMotion
    ? undefined
    : {
        transitionProperty: "opacity, transform",
        transitionDuration: `${SYNC_TRANSITION_MS}ms`,
        transitionTimingFunction: easePanel
      };

  const useFixedShell = dock != null && isFiltersOpen;

  const scrollShellClass = [
    "hidden min-h-0 lg:block",
    useFixedShell
      ? "lg:fixed lg:z-[35] lg:overflow-y-auto lg:overscroll-contain lg:pr-1 lg:pt-[1in] lg:pb-[1in]"
      : "lg:sticky lg:top-28 lg:z-[1] lg:max-h-[calc(100dvh-7.5rem)] lg:overflow-y-auto lg:overscroll-contain lg:pr-1"
  ].join(" ");

  const scrollShellStyle:
    | {
        left: number;
        width: number;
        top: number;
        bottom: number;
      }
    | undefined =
    useFixedShell && dock
      ? {
          left: dock.left,
          width: dock.width,
          top: dock.top,
          bottom: dock.bottom
        }
      : undefined;

  return (
    <aside
      ref={asideRef}
      id="shop-desktop-filters"
      className={[
        "order-2 hidden min-w-0 shrink-0 lg:order-1 lg:block lg:self-stretch",
        isFiltersOpen
          ? "lg:w-[280px] lg:max-w-[280px] xl:w-[300px] xl:max-w-[300px]"
          : "lg:w-0 lg:max-w-0 lg:-mr-8"
      ].join(" ")}
      style={columnStyle}
      aria-hidden={!isFiltersOpen}
    >
      <div className="relative h-full min-h-0">
        <div className={scrollShellClass} style={scrollShellStyle}>
          <div
            className={[
              "rounded-2xl border border-mc-ink/10 bg-white/95 p-4 shadow-sm ring-1 ring-mc-ink/[0.04] backdrop-blur-md",
              isFiltersOpen ? "pointer-events-auto" : "pointer-events-none",
              isFiltersOpen ? "translate-x-0 opacity-100" : "-translate-x-3 opacity-0"
            ].join(" ")}
            style={panelStyle}
          >
            <div className="flex items-start justify-between gap-2">
              <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-mc-gold">Refine</p>
              <button
                type="button"
                onClick={closeFilters}
                className="-m-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-mc-ink/70 transition hover:bg-mc-ink/5 hover:text-mc-ink"
                aria-label="Close filters"
              >
                <X className="h-4 w-4" aria-hidden />
              </button>
            </div>
            <div className="mt-3">
              <ShopFilters
                options={options}
                basePath={basePath}
                enablePriceSlider={enablePriceSlider}
                hideOutOfStockToggle={hideOutOfStockToggle}
                layout="stack"
                omitSort
                compact
                deferUrlUntilApply
                applyFiltersRef={applyFiltersRef}
                showApplyButton
                preserveQueryKeys={preserveQueryKeys}
                omitUrlFilterKeys={omitUrlFilterKeys}
                hideCategoryFilter={hideCategoryFilter}
                hideFacetFields={hideFacetFields}
                hideClearButton={false}
                priceSliderCommitOnChange={false}
              />
            </div>
          </div>
        </div>
      </div>
    </aside>
  );
}
